...
class EntityConstructor
{
     ...
     */
    public static function compileEntityDataClass(
        string $className,
        string $tableName,
    ...
    public static function getNewFieldsMap(array $fieldsMap): array
    {
        ...
        foreach ($fieldsMap as $columnName => $columnInfo) {
            ...
            switch ($columnInfo['data_type']) {
                case 'integer':
                    $newFieldsMap[] = 'new Main\Entity\IntegerField(\'' . $columnName . '\', ' . var_export($params, true) . ')';
                    break;
                case 'float':
                    $newFieldsMap[] = 'new Main\Entity\FloatField(\'' . $columnName . '\', ' . var_export($params, true) . ')';
                    break;
                case 'boolean':
                    $newFieldsMap[] = 'new Main\Entity\BooleanField(\'' . $columnName . '\', ' . var_export($params, true) . ')';
                    break;
                case 'date':
                    $newFieldsMap[] = 'new Main\Entity\DateField(\'' . $columnName . '\', ' . var_export($params, true) . ')';
                    break;
                case 'datetime':
                    $newFieldsMap[] = 'new Main\Entity\DateTimeField(\'' . $columnName . '\', ' . var_export($params, true) . ')';
                    break;
                case 'string':
                    if ($columnInfo['length'] > 255) {
                        $newFieldsMap[] = 'new Main\Entity\TextField(\'' . $columnName . '\', ' . var_export($params, true) . ')';
                    } else {
                        $newFieldsMap[] = 'new Main\Entity\StringField(\'' . $columnName . '\', ' . var_export($params, true) . ')';
                    }
                    break;
